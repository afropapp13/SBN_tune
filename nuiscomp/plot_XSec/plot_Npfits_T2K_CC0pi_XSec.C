...
TString get_chi2(TH1D *hData, TH1D *hMC, TH2D *cov, TH1D *hChi2) { 
...
void plot_Npfits_T2K_CC0pi_XSec() {
	...
	ofstream outfile;
        outfile.open(nuiscomp + "T2K_CC0pi/parval_Npfits.txt");
	for (int par=0; par<npar; par++) {
		if (par == npar-1) {outfile<<parameter_vector[par]<<endl;}
		else {outfile<<parameter_vector[par]<<"\t";}
	}

	// Set up plot
	TCanvas *main_can = new TCanvas();
	TCanvas *extra_can = new TCanvas();
	THStack *stk;
	THStack *chi2_stk;
	...
	for (int par=0; par<npar; par++) {
		...
		// Import hists
		TString infilename = nuismin +  "nuismin_"+(to_string(par+1)).c_str()+"p.root";
	        TFile *infile = new TFile(infilename, "read");

		// Plot the fit parameter covariance matrix
		if (par > 0) {
			TH2D *par_cov = (TH2D *)infile->Get("covariance")->Clone("par_cov");
			par_cov->SetTitle("");
			par_cov->SetStats(0);
			extra_can->cd();
			par_cov->Draw("colz text");
			extra_can->Print(nuiscomp_plot_xsec + "T2K_CC0pi/analysis/Npfits_par_cov_"+(to_string(par+1)).c_str()+"p.png");
		}
		...
		// Save fit values
	        ...
	        myReader.Next();
		if (par_vals.GetSize() != par+1) {
			cout<<"Found "<<par_vals.GetSize()<<" parameter fits, but was expecting "<<par+1<<endl;
			return;
		}
		for (int i=0; i<par+1; i++) {
			if (i == par) {outfile<<par_vals[i]<<endl;}
			else {outfile<<par_vals[i]<<"\t";}
		}	
		...
		if (par == 0) {
			...
			chi2_entry_prefit = get_chi2(hData, hMC_prefit, cov_prefit, hChi2_prefit);

			// Plot the t2k covariance matrix 
			//cov_prefit->SetStats(0);
			//extra_can->cd();
			//cov_prefit->Draw("colz");
			//extra_can->SetRightMargin(0.13);
			//extra_can->Print("/exp/dune/app/users/jwolfs/t2k_tune/outpngs/plot_XSec/T2K_CC0pi/analysis"+analysis
			//	+"/Npfits_t2k_cov.png");
	
		}
	        cov_postfit = (TH2D *)infile->Get("T2K_CC0pi_XSec_2DPcos_nu_"+analysis+"_INVCOV")->Clone("cov_postfit");
		chi2_entry_postfit = get_chi2(hData, hMC_postfit, cov_postfit, hChi2_postfit);
		...
		if (dashed_line) {hMC_postfit->SetLineStyle(7);}
	        stk->Add(hMC_postfit);
		TString leg_entry = (to_string(par+1)+"p Tune").c_str();
	        leg->AddEntry(hMC_postfit, leg_entry+" ("+chi2_entry_postfit+")", "l");
		...
		chi2_stk->Add(hChi2_postfit);
	        chi2_leg->AddEntry(hChi2_postfit, leg_entry+" ("+chi2_entry_postfit+")", "l"); 
	...
	main_can->cd();
        ...
        leg->Draw();
	main_can->Print("/exp/dune/app/users/jwolfs/t2k_tune/outpngs/plot_XSec/T2K_CC0pi/analysis"+analysis
		+"/Npfits_xsec.png");
