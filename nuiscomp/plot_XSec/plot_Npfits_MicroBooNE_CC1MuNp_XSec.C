...
void plot_Npfits_MicroBooNE_CC1MuNp_XSec() {
	...
	for (TString var: var_vector) {
		...
		for (int par=0; par<npar; par++) {
			...
			TString infilename = "/exp/dune/app/users/jwolfs/t2k_tune/outfiles/nuiscomp/MicroBooNE_CC1MuNp/analysis"+analysis+
				"/nuiscomp_"+(to_string(par+1)).c_str()+"p.root";
			...
			if (par == 0) {
				...
				chi2_entry_prefit = get_chi2(hData, hMC_prefit, cov_prefit);
	                        ...
	                        //extra_can->Print("/exp/dune/app/users/jwolfs/t2k_tune/outpngs/plot_XSec/MicroBooNE_CC1MuNp/analysis"+analysis
	                        //      +"/Npfits_ub_cov.png");
			...
			hMC_postfit->SetLineWidth(1);
	        	stk->Add(hMC_postfit);
			TString leg_entry = (to_string(par+1)+"p Tune ").c_str()+analysis;
		...
		main_can->cd();
	        ...
	        leg->Draw();
	        main_can->Print("/exp/dune/app/users/jwolfs/t2k_tune/outpngs/plot_XSec/MicroBooNE_CC1MuNp/analysis"+analysis
			+"/Npfits_MicroBooNE_CC1MuNp_XSec_1D"+var+"_nu.png");
