...
void plot_Npfits_MicroBooNE_CC1Mu1p_XSec() {
	...
	for (TString var: var_vector) {
		...
		for (int par=0; par<npar; par++) {
			...
			if (par+1 == 3) {continue;} // PUT IT BACK!
			if (par+1 == 4) {continue;} // PUT IT BACK!


			// Import hists
			TString infilename = nuiscomp + "/MicroBooNE_CC1p/nuiscomp_"+(to_string(par+1)).c_str()+"p.root";
		...
		main_can->cd();
	        ...
	        leg->Draw();
	        main_can->Print(nuiscomp_plot_xsec + "MicroBooNE_CC1p/Npfits_MicroBooNE_CC1Mu1p_XSec_1D"+var+"_nu.pdf");
